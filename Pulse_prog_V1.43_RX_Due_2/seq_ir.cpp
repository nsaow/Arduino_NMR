...
//  setEvent(ID, PortC_State(Trigger, I, Q, Read, transfer) , rc, nextID);
    ...
    setEvent(j+2, PortC_State(0,1,0,0,0), Tp180, j+3);  
    setEvent(j+3, PortC_State(0,0,0,0,0), IRtau - Tp180/2 - Tp90/2 + (double)i*IRtau, j+4);  
    setEvent(j+4, PortC_State(0,1,0,0,0), Tp90, j+5);      
    setEvent(j+5, PortC_State(0,0,0,0,0), Tdeadtime, j+6);  
    setEvent(j+6, PortC_State(0,0,0,1,0), TreadoutF, j+7);  
    setEvent(j+7, PortC_State(0,0,0,0,1), T_transferF, j+8);  
    setEvent(j+8, PortC_State(0,0,0,0,0), Tres - (double)i*IRtau, j+9); 
  }
