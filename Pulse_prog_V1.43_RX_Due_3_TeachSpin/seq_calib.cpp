...
//  setEvent(ID, PortC_State(Trigger, I, Q, Read, transfer) , rc, nextID);
    ...
    if (iq == 0) 
      setEvent(j+2, PortC_State(0,1,0,1,0), Tpw0 + (double)i*Tdpw, j+3);
    else if (iq == 1)
      setEvent(j+2, PortC_State(0,0,1,1,0), Tpw0 + (double)i*Tdpw, j+3);
    setEvent(j+3, PortC_State(0,0,0,1,0), Tdeadtime, j+4);
    setEvent(j+4, PortC_State(0,0,0,0,0), TreadoutF, j+5);
    setEvent(j+5, PortC_State(0,0,0,1,1), T_transferF, j+6);
    setEvent(j+6, PortC_State(0,0,0,1,0), Tres - (double)i*Tdpw, j+7);
  }
